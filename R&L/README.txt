...
- stm.h/stm.cpp: is the library that computes the state transition matrix of a given 3BP with state vector x. In particular this routine gives the differential equations that the STM has to satisfy. So, for example, for a 3D system, this library returns a vector "derivatives" that has, in order of rows, the equations of each component of the stm.

derivative[0]=dphi/dt(element 1,1)
	.
	.
	.
derivative[36]=dphi/dt(element 6,6)
